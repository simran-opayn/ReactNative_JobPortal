...
const JobFilterModal = ({onPressSubmit = () => {}, onPressCancel = () => {}}) => {
            ...
            <View style={styles.overlaycontainer}>
                <View style={styles.sectionview}>
                    ...
                    <View style={{marginVertical: 16}}>
                        <Text style={[CustomStyling.medium16Text]}>Job Level</Text>
                        <View style={{flexDirection: "row", flexWrap: "wrap", marginTop: 4, width: "100%" }}>
                        {JobPositions.map((item, index) => ( 
                            <TouchableOpacity style={{borderRadius: 12, backgroundColor: (formData?.jobLevel == item.id) ? color.mainColor : color.white, paddingHorizontal: 8, paddingVertical: 4, marginVertical: 4, marginRight: 8, borderWidth: 1}}
                                onPress={() => onTextChange("jobLevel", item.id)}
                            >
                                <Text style={[CustomStyling.regular16Text, {color: (formData?.jobLevel == item.id) ? color.white : color.subtitleBlack}]}>{item.name}</Text>
                            </TouchableOpacity>
                        ))}
                        </View>
                    </View>
                    ...
                    <View>
                        ...
                        <RangeSlider
                            ...
                            onTouchEnd={(low, high) => {
                                ...
                                setLowRange(low);
                                setHighRange(high);
                                // onTextChange("highSalary", low);
                                console.log("USD"+lowRange+"     USD"+highRange);
                            }}
                        />
                        <View style={{flexDirection: 'row', justifyContent: 'space-between', width: "100%"}}> 
                            <Text style={CustomStyling.medium16Text}>${lowRange}</Text>
                            <Text style={CustomStyling.medium16Text}>${highRange}</Text>
                        </View>
