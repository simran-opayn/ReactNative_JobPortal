...
const SetNewPasswordView = ({navigation=useNavigation()}) => {
    const [showErrorMsg, setShowErrMsg] = useState(false);

    const SetPasswordValidationSchema = yup.object().shape({
        otp: yup
            .string()
            .min(4, ({ min }) => `Please enter the 4-digit code recive by email`)
    ...
    return(
        ...
        <View style={{backgroundColor: color.mainColor, flex: 1}}>
            <Text style={[AuthStyle.viewTitle]}>Reset Password</Text>
        <ScrollView style={[CustomStyling.curveViewStyle, {marginTop: 40}]}>
            ...
            <CustomTextField 
                ...
                showError={showErrorMsg && errors.otp}
